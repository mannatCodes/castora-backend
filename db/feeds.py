...
def mark_source_feed_crawled(sources_db_path, feed_id, timestamp=None):
    query = """
    UPDATE source_feeds
    SET last_crawled = ?
    WHERE id = ?
    """
    return execute_query(sources_db_path, query, (timestamp or datetime.now().isoformat(), feed_id))
