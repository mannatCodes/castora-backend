...
def init_tasks_db():
    ...
    with db_connection(db_path) as conn:
        ...
        for index_sql in indexes:
            cursor.execute(index_sql)
        cursor.execute("SELECT COUNT(*) as count FROM tasks WHERE task_type = 'feed_processor'")
        feed_processor_exists = cursor.fetchone()["count"] > 0
        if not feed_processor_exists:
            cursor.execute(
                """
                INSERT INTO tasks
                (name, task_type, description, command, frequency, frequency_unit, enabled, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    "Feed Processor",
                    "feed_processor",
                    "Processes RSS feeds and stores new entries",
                    "python -m processors.feed_processor",
                    1,
                    "hours",
                    1,
...
async def init_agent_session_db():
    """Initialize the agent session database."""
    start_time = time.time()
    db_path = get_db_path("agent_session_db")
    with db_connection(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("BEGIN TRANSACTION")
        cursor.execute("""
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_podcast_sessions_session_id ON podcast_sessions(session_id)")
        conn.commit()
    elapsed = time.time() - start_time
    print(f"Agent session database initialized in {elapsed:.3f}s")
...
async def init_databases():
    ...
    for db_name in ["sources_db", "tracking_db", "podcasts_db", "tasks_db", "agent_session_db"]:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        tasks = [
            ...
            # init_agent_session_db is an async coroutine; run it properly in the executor
            loop.run_in_executor(executor, lambda: asyncio.run(init_agent_session_db())),
        ]
