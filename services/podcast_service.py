...
class PodcastService:
    ...
    async def get_podcasts(
        ...
        try:
            ...
            filter_params = []
            if search:
                where_conditions.append("(title LIKE ? OR content_json LIKE ? OR sources_json LIKE ?)")
                search_param = f"%{search}%"
                filter_params.extend([search_param, search_param, search_param])
            if date_from:
            ...
            if language_code:
                ...
                filter_params.append(language_code)
            if tts_engine:
                where_conditions.append("tts_engine = ?")
                filter_params.append(tts_engine)
            if has_audio is not None:
                where_conditions.append("audio_generated = ?")
                filter_params.append(1 if has_audio else 0)
            if where_conditions:
            ...
            if search:
                ...
                ORDER BY
                    ...
                    created_at DESC
                """
                params.extend([search, f"{search}%", search_param])
            else:
                query += " ORDER BY date DESC, created_at DESC"
            query += " LIMIT ? OFFSET ?"
            params.extend([per_page, offset])
            total_result = await podcasts_db.execute_query(count_query, tuple(filter_params), fetch=True, fetch_one=True)
