...
from processors.feed_processor import fetch_and_process_feeds
from processors.url_processor import crawl_pending_entries
...
class SourceService:
    ...
    @staticmethod
    def _looks_like_feed_url(url: Optional[str]) -> bool:
        if not url:
            return False
        lowered = url.lower()
        return any(marker in lowered for marker in ("rss", "feed", ".xml", ".rss", "atom"))

    async def _ensure_source_url_feed(self, source_id: int, url: Optional[str]) -> None:
        """Treat an RSS-looking source URL as the source's main feed."""
        if not self._looks_like_feed_url(url):
            return
        existing_feed = await sources_db.execute_query(
            "SELECT id FROM source_feeds WHERE source_id = ? AND feed_url = ?",
            (source_id, url),
            fetch=True,
            fetch_one=True,
        )
        if existing_feed:
            return
        feed_data = SourceFeedCreate(feed_url=url, feed_type="main", is_active=True)
        await self.add_feed_to_source(source_id, feed_data, ingest=False)

    async def _sync_source_feeds(self, source_id: int, feeds: Optional[List[SourceFeedCreate]]) -> None:
        if feeds is None:
            return
        existing_feeds = await self.get_source_feeds(source_id)
        existing_by_id = {feed["id"]: feed for feed in existing_feeds}
        existing_by_url = {feed["feed_url"]: feed for feed in existing_feeds}
        seen_feed_ids = set()
        for feed in feeds:
            feed_id = getattr(feed, "id", None)
            feed_url = (feed.feed_url or "").strip()
            if not feed_url:
                continue
            matching_feed = existing_by_id.get(feed_id) if feed_id else existing_by_url.get(feed_url)
            if matching_feed:
                ...
                seen_feed_ids.add(matching_feed["id"])
            else:
                await self.add_feed_to_source(source_id, feed, ingest=False)
        for feed in existing_feeds:
            if feed["id"] not in seen_feed_ids and not any((f.feed_url or "").strip() == feed["feed_url"] for f in feeds):
                await self.delete_feed(feed["id"])

    def _ingest_new_feed_entries(self) -> None:
        """Best-effort immediate ingest so newly added RSS feeds show articles quickly."""
        try:
            ...
            crawl_pending_entries(tracking_db_path=get_tracking_db_path(), batch_size=50)
        except Exception as exc:
            print(f"Warning: immediate source ingest failed: {exc}")
    ...
    async def create_source(self, source_data: SourceCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            if source_data.feeds:
                ...
                await self._ensure_source_url_feed(source_id, source_data.url)
            self._ingest_new_feed_entries()
        ...
        try:
            ...
            if source_data.categories is not None:
                ...
                if source_data.category:
                    await self.add_source_category(source_id, source_data.category)
            await self._sync_source_feeds(source_id, source_data.feeds)
            await self._ensure_source_url_feed(source_id, source_data.url)
            if source_data.feeds is not None or self._looks_like_feed_url(source_data.url):
                self._ingest_new_feed_entries()
            return await self.get_source(source_id)
        except Exception as e:
            if isinstance(e, HTTPException):
    ...
    async def add_feed_to_source(self, source_id: int, feed_data: SourceFeedCreate, ingest: bool = True) -> Dict[str, Any]:
