...
class AudioGenerationTests(unittest.TestCase):
    ...
    def test_usable_short_audio_is_extended_to_required_duration(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            audio_path = os.path.join(tmp_dir, "short-but-usable.wav")
            _write_tone(audio_path, duration_seconds=41.0)

            extended = audio_generate_agent._extend_audio_to_minimum_duration(audio_path, 65.3)
