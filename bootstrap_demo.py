"""Bootstrap Castora demo data into local working folders."""
...
def ensure_empty(dir_path):
    """Check if a directory is empty, or create it."""
    if os.path.exists(dir_path):
        if os.listdir(dir_path):
            print(f"X '{dir_path}' is not empty. aborting.")
            sys.exit(1)
    else:
...
def download_file(url, dest_path):
    ...
    print("Downloading Castora demo content...")
...
def extract_zip(zip_path, extract_to):
    """Extract zip file into extract_to (project root) with a progress bar."""
