...
SEARCH_AGENT_INSTRUCTIONS = dedent("""
    ...
    IMPORTANT: Make sure you are able to detect what tool to use and use it available tool tags = ["google_news_discovery", "duckduckgo", "wikipedia_search", "jikan_search", "browser_search", "unknown"].
...
def search_agent_run(agent: Agent, query: str) -> str:
    ...
    response = run_with_retry_and_throttle(
        lambda: search_agent.run(query, session_id=session_id),
        api_name="groq",
        max_retries=3,
        base_delay=2
    )
