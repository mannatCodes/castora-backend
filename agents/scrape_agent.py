...
def crawl_urls_batch(search_results):
    ...
    if unique_urls:
        try:
            ...
            scraped_results = browser_crawler.scrape_urls(unique_urls) or []
        except Exception as e:
            print(f"Browser crawler failed, using search descriptions as fallback: {e}")
    ...
    for result in scraped_results:
        original_url = result.get("original_url") or result.get("originalUrl")
        if original_url:
            url_to_scraped[original_url] = result
    ...
    for _, search_result in enumerate(search_results):
        ...
        try:
            ...
            response = run_with_retry_and_throttle(
                lambda: scrape_agent.run(verification_prompt, session_id=agent.session_id),
                api_name="groq",
                max_retries=3,
                base_delay=2
            )
...
def scrape_agent_run(
    ...
    updated_results, _, _ = crawl_urls_batch(current_state.get("search_results", []))
