...
def process_article_with_ai(client, article, max_tokens=5000):
...
def analyze_articles(tracking_db_path=None, groq_api_key=None, batch_size=5, delay_range=(1, 3)):
    ...
    for i, article in enumerate(articles):
        ...
        if success:
            ...
            if error_message and "rate_limit" in error_message.lower():
                print("Rate limit reached; stopping this batch so remaining articles can retry later.")
                break
...
if __name__ == "__main__":
    ...
    if not api_key:
        print("Error: No Groq API key provided. Please provide via --api_key or set Groq_API_KEY in .env file")
        exit(1)
