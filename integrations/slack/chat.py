...
async def poll_for_completion(session_id: str, thread_key: str, task_id=None):
    ...
    active_sessions[session_id]["thread_key"] = thread_key
    active_sessions[session_id]["task_id"] = task_id
    ...
    try:
        while poll_count < max_polls:
            try:
                ...
                completed = (
                    not status_response.get("is_processing", True)
                    or status_response.get("stage") in ["script", "banner", "audio", "complete"]
                )
...
def start_background_polling(session_id: str, thread_key: str, task_id=None):
    ...
    if session_id in active_sessions:
        ...
        if future and not future.done():
            print(f"Polling already active for {session_id}")
            return
...
async def process_source_confirmation(thread_key: str, message: str):
    try:
        ...
        if (
             chat_response.get("is_processing")
